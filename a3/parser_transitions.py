...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if "S" == transition:
            ...
            # 不可以因为直接改变了sentence,代表python在储存空间中都是指向了原地址
            #del self.buffer[0]
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        for i in range(len(minibatch_parses)):
            minibatch_parses[i].parse([transitions[i]])
            if len(minibatch_parses[i].buffer) == 0 and len(minibatch_parses[i].stack) == 1:
                #dependencies.append(minibatch_parses[i].dependencies)
                unfinished_parses.remove(minibatch_parses[i])

    #dependencies = [minibatch_parses[i].dependencies for i in range(len(minibatch_parses)) for minibatch_parses[i] in partial_parses] 
