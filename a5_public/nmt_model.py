...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2, no_char_decoder=False):
        ...
        # h*2h (W) * 2h*1(h_enc) = h*1 --> input = 2h output = h
        self.h_projection = nn.Linear(hidden_size * 2, hidden_size, bias = False)
        # h*2h (W) * 2h*1(c_enc) = h*1 --> input = 2h output = h
        self.c_projection = nn.Linear(hidden_size * 2, hidden_size, bias = False)
        # 1*h(h_dec_t) * h*2h(W) * 2h*1(h_enc_i) = 1*1
        self.att_projection = nn.Linear(2 * hidden_size, hidden_size, bias = False)
        # h*3h(W) * 3h *1(u_t) = h*1 --> input = 3h output = h
        self.combined_output_projection = nn.Linear(3 * hidden_size, hidden_size, bias = False)
        # V_t *h(W) * h*1(o_t) = V_t * 1 --> input=h output=V_t
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device=self.device) # src_max_len, b, max_word_len
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(target, device=self.device) # tgt_max_len, b, max_word_len
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens) # (max_len, b, h* 2)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # 1.
        enc_hiddens_proj = self.att_projection(enc_hiddens) # (b, 1, h*2) * (h * 2, h) --> b, 1, h
        # 2.
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ### COPY OVER YOUR CODE FROM ASSIGNMENT 4
        # 1. dec_state check need
        dec_state = self.decoder(Ybar_t, dec_state)
        (dec_hidden, dec_cell) = dec_state
        # 3. (b, src_len, h) * (b, h, 1) --> (b, src_len, 1) -->(b, src_len)
        ...
        # 2. (b, src_len) -> (b, 1, src_len) bmm (b, src_len, 2h) -->(b, 1, 2h) --> (b, 2h)
        a_t = alpha_t.unsqueeze(1).bmm(enc_hiddens).squeeze(1)
        # 3. a_t (b, 2h) dec_hidden(b, h) U_t(d, 3h)
