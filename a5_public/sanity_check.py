...
def reinitialize_layers(model):
    """
    initial the layer weights --> forward pass check
    use highway directly --> use torch directly
    """
    def init_weights(m):
        if type(m) == nn.Linear:
...
def generated_initial_data():
    """
    build the highway part by numpy
    """
...
def question_1h_sanity_check(highway):
    ...
    # shape check (real output == expected output?)
    # real output = highway(input)
    # input = initial test data
    # 1. initial test input data --> generate_initial_data
    initial_inpt, output_expected = generated_initial_data()

    initial_inpt = torch.Tensor(initial_inpt)
    output_expected = torch.Tensor(output_expected)

    with torch.no_grad():
        outp = highway(initial_inpt)

    output_expected_size = (BATCH_SIZE, EMBED_SIZE)
    assert(outp.numpy().shape == output_expected_size),\
    "highway output shape is incorrect it should be :{} but is:{}".format(output_expected_size, outp.numpy().shape)

    assert(np.allclose(output_expected.numpy(), outp.numpy())),\
    "highway forward pass is incorrect: it shoule be {} but is{}".format(output_expected, outp)

    print("passed shape check and forward pass check {} !!! :D".format(":I"))

def question_1i_sanity_check(cnn):
    """
    sanity check for cnn.py, basic shape check
    """

    reinitialize_layers(cnn)
    inpt = np.random.rand(BATCH_SIZE, EMBED_CHAR_SIZE, MAX_WORD_LENGTH)
    ...
    output_expected_size = (BATCH_SIZE, EMBED_SIZE)
    assert (oupt.numpy().shape == output_expected_size), \
        "cnn output shape is incorrect it should be :{} but is:{}".format(output_expected_size, oupt.numpy().shape)

    print("passed shape check {} !!! :D".format("1i"))
...
def main():
    ...
    # Initialize CNN
    # PAY ATTENTION kernel_num_filters = word_embed_size
    cnn = CNN(
        num_filter = EMBED_SIZE,
        char_embed_size = EMBED_CHAR_SIZE,
