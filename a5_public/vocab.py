...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sent in sents:
            char_sent_idx = []
            for word in sent:
                char_word_index = [self.start_of_word] + [self.char2id[cha] for cha in word] + [self.end_of_word]
                char_sent_idx.append(char_word_index)
            word_ids.append(char_sent_idx)
        return word_ids
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        pad_res = pad_sents_char(charindices, self['<pad>'])  # (batch_size, max_sentence_length, max_word_length)
        sents_var = torch.tensor(pad_res).permute(1, 0, 2).to(device)  # (max_sentence_length, batch_size, max_word_length)
        return sents_var
